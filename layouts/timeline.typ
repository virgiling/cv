#let layout-timeline(
  ...
  block(width: 100%, breakable: isbreakable, inset: 0pt, outset: 0pt)[
    ...
    #for (i, entry) in data.enumerate() {
      ...
      } else if "excepted-date" in entry and entry.excepted-date != none {
        [
          #entry.start-date - #entry.excepted-date \
          #text(size: 0.7em, style: "italic")[(expected)]
        ]
      ...
      grid(
        ...
        // Year column
        align(right)[#text(style: "italic", year-text)],
          ...
          #for field in tertiary {
            if field in entry and entry.at(field) != none {
              ...
              if field == "description" {
                // 对于description字段，只显示value并用斜体
                tertiary-content.push(emph[#value])
              } else {
                // 其他字段显示 key: value 格式
                tertiary-content.push(emph[#field: #value])
              }
            }
          }
